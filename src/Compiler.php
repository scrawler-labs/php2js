...
class Compiler extends PrettyPrinterAbstract
{
    ...
    {
        throw new \LogicException('Magic constants are not supported !');
    }
    ...
    {
        if(! in_array($node->var->name,$this->vars)){
        array_push($this->vars,$node->var->name);
        $node->var->name = 'let '. $node->var->name;
        }
    ...
    {
        return $this->pPrefixOp(Expr\Print_::class, 'console.log ', $node->expr);
    }
    ...
    {
        return 'console.log( ' . $this->pCommaSeparated($node->exprs) . ');';
    }
